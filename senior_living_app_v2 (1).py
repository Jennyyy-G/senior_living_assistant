...
with st.sidebar:
    ...
    with st.expander("🔍 Debug Info"):
        ...
        st.write(f"Has Audio: {st.session_state.audio_files is not None}")
        st.write(f"Has Transcription: {st.session_state.transcription is not None}")
        st.write(f"Has Preferences: {st.session_state.preferences is not None}")
        st.write(f"Has Results: {st.session_state.results is not None}")
...
if st.session_state.step == "upload":
    ...
    if audio:
        ...
        st.success(f"✅ {audio.name} uploaded successfully")
...
if st.session_state.step == "transcribe":
    ...
    with st.spinner("🎧 Transcribing audio..."):
        try:
            ...
            st.info(f"📁 File: {audio_file.name} ({file_size_mb:.2f} MB)")
            
            if file_size_mb > 25:
                st.error("❌ File too large. OpenAI Whisper supports files up to 25MB.")
                st.stop()
            ...
            st.info("🔄 Sending to OpenAI Whisper API...")
            ...
            if isinstance(transcript, str):
                ...
                st.session_state.transcription = transcript.text if hasattr(transcript, 'text') else str(transcript)
            
            if not st.session_state.transcription or len(st.session_state.transcription.strip()) < 10:
                st.error("❌ Transcription returned empty or very short text. Please check your audio file.")
                st.stop()
            ...
            if "api key" in error_msg or "authentication" in error_msg or "401" in error_msg:
                st.error("🔑 **API Key Issue**: Your OpenAI API key appears to be invalid or missing.")
                st.info("Please check:\n- Key is correctly copied (no spaces)\n- Key has not expired\n- Account has API access enabled")
            
            elif "connection" in error_msg or "timeout" in error_msg or "network" in error_msg:
                st.error("🌐 **Network Issue**: Cannot connect to OpenAI servers.")
                ...
                st.error("💳 **Quota Issue**: API quota exceeded or billing problem.")
                st.info("Please check:\n- OpenAI account has credits\n- Billing information is up to date\n- Rate limits have not been exceeded")
            
            elif "file" in error_msg or "format" in error_msg:
                st.error("📁 **File Issue**: Problem with audio file format.")
                st.info("Please check:\n- File is a valid audio format (m4a, mp3, wav, mp4)\n- File is not corrupted\n- File size is under 25MB")
...
if st.session_state.step == "preferences":
    ...
    with st.spinner("🔍 Analyzing client preferences..."):
        try:
            client = OpenAI(api_key=api_key)

            system_prompt = """
            You are a JSON generator for senior living placement.
            You MUST output ONLY valid JSON with NO markdown, NO explanations.
            ...
            st.subheader("📋 Extracted Client Preferences")
            st.json(prefs)
            ...
            st.write("Raw response:", raw if 'raw' in locals() else "No response")
...
if st.session_state.step == "rank":
    ...
    with st.spinner("🔄 Loading and filtering communities..."):
        try:
            ...
            st.write(f"📊 Loaded {initial_count} communities from database")
            ...
            st.success(f"✅ Processing complete! {len(df)} communities matched and ranked")
